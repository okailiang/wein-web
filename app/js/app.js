...
App.run(["$rootScope", "$state", "$stateParams", '$window', '$templateCache', function ($rootScope, $state, $stateParams, $window, $templateCache) {
    ...
    $rootScope.app = {
        name: 'wein',
        hostUrl: 'http://wein.press:8080',
        description: 'wein专业打印',
    ...
    function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
        ...
        $stateProvider
            ...
            .state('app.printShopInfo', {
                url: '/printShopInfo/:id',
                title: '打印店新增和编辑',
                templateUrl: helper.basepath('print-shop-info.html'),
                resolve: helper.resolveFor('toaster', 'ngDialog', 'loaders.css'),
            })
            .state('app.printShopDetail', {
                url: '/printShopDetail/:id',
                title: '打印店详情',
                templateUrl: helper.basepath('print-shop-detail.html'),
                resolve: helper.resolveFor('toaster', 'ngDialog', 'loaders.css'),
                controller: ["$rootScope", "$scope", "$http", 'toaster', '$stateParams', function ($rootScope, $scope, $http, toaster, $stateParams) {
                    /**
                     * 详情
                    ...
                    $scope.getPrintShop = function (id) {
                        var url = '/admin/printShop/getPrintShop?id=' + id;
                        $http.get(url).success(function (data) {
                    ...
                    $scope.getPrintShop($stateParams.id);
...
App.controller('RegisterFormController', ['$rootScope', '$scope', '$http', '$state', 'toaster', '$timeout', '$interval', function ($rootScope, $scope, $http, $state, toaster, $timeout, $interval) {

    $scope.code_text = "获取验证码";
    $scope.register_submmit = false;
    // bind here all data from the form
    $scope.account = {};
    // place the message if something goes wrong
    $scope.authMsg = '';
    ...
    $scope.getRegisterCode = function () {
        $scope.show_code = false;//置灰获取验证码按钮
        ...
        var timerHandler = $interval(function () {
            if (second <= 0) {
                ...
                $scope.code_text = second + "秒后重发";
                second--;
                $scope.show_code = true;
            }
        }, 1000);

        var registerCodeURL = $rootScope.app.hostUrl + '/send/emailCode?time=' + (new Date().getTime()) + '&email=' + $scope.register.email;

        $http.get(registerCodeURL).success(function (data) {
    ...
    $scope.register = function () {
        ...
        if ($scope.registerForm.$valid) {
            $scope.register_submmit = true;
            var data = {
                userName: $scope.register.userName,
                account: $scope.register.email,
                password: $scope.register.password,
                registerCode: $scope.register.registerCode
            };
            $http
                .post('/register', data)
            ...
            $scope.registerForm.account_registerCode.$dirty = true;
        }
    };
...
App.controller('RecoverFormController', ['$rootScope', '$scope', '$http', '$state', 'toaster', '$timeout', '$interval', function ($rootScope, $scope, $http, $state, toaster, $timeout, $interval) {

    //初始化
    $scope.recoverForm_show = true;
    $scope.resetPasswordForm_show = false;

    $scope.code_text = "获取验证码";
    $scope.recover_submmit = false;
    $scope.resetPassword_submmit = false;
    // place the message if something goes wrong
    $scope.authMsg = '';

    //验证发送校验码
    $scope.getRecoverCode = function () {
        $scope.show_code = false;//置灰获取验证码按钮
        if ($scope.recover.email == undefined) {
            $scope.recoverForm.recover_email.$dirty = true;
            return;
        }
        //控制60秒后重发
        var second = 60;
        ...
        var recoverCodeURL = '/resetPassword/code?time=' + (new Date().getTime()) + '&email=' + $scope.recover.email;

        $http.get(recoverCodeURL).success(function (data) {
            toaster.pop('success', null, data.message);
        }).error(function (data, status, headers, config) {
            $interval.cancel(timerHandler);
            second = 59;
    ...
    //下一步
    $scope.userId = 0;
    $scope.recoverNext = function () {
        if ($scope.recoverForm.$valid) {
            var recoverCodeURL = '/resetPassword/checkCode?v=' + (new Date().getTime())
                + '&email=' + $scope.recover.email
                + '&code=' + $scope.recover.recoverCode;
            $http.get(recoverCodeURL).success(function (data) {
                //toaster.pop('success', null, data.message);
                $scope.userId = data.id;
                $scope.recover.email = data.email;
                $scope.recoverForm_show = false;
                $scope.resetPasswordForm_show = true;
            }).error(function (data, status, headers, config) {
                toaster.pop('error', null, data.message);
            });
        } else {
            $scope.recoverForm.recover_email.$dirty = true;
            $scope.recoverForm.recover_code.$dirty = true;
        }
    };
    ...
    $scope.resetPassword = function () {
        ...
        if ($scope.resetPasswordForm.$valid && $scope.userId > 0) {
            $scope.resetPassword_submmit = true;
            var data = {
                id: $scope.userId,
                email: $scope.recover.email,
                password: $scope.recover.password
            };
            $http
            ...
            $scope.resetPasswordForm.account_password.$dirty = true;
        }
    };
...
App.controller('DataTableController', ['$http', '$scope', '$resource', 'DTOptionsBuilder', 'DTColumnDefBuilder',
    function ($http, $scope, $resource, DTOptionsBuilder, DTColumnDefBuilder) {
        'use strict';

        // Ajax
        //role/listRoles    server/datatable.json
        // $http.get('/role/listRoles').success(function(data) {debugger
        //   $scope.persons = data.resultList;
        //   }).error(function(data, status, headers, config) {
        //       debugger

        //   });
        $resource('/role/listRoles').query().$promise.then(function (persons) {
            debugger
            $scope.persons = persons.resultList;
        });
// $http.get('/role/listRoles').success(function(data) {debugger
//     $scope.persons = data.resultList;
//     }).error(function(data, status, headers, config) {
...
App.controller('RoleController', ['$scope', '$http', 'toaster', 'ngDialog', function ($scope, $http, toaster, ngDialog) {
    ...
    $scope.listRoles = function () {
        ...
        var listParam = '?time=' + (new Date().getTime())
            + "&id=" + $scope.handleParam($scope.searchRoleId) + "&roleName=" + $scope.handleParam($scope.searchRoleName);
        console.log(listParam);
        $http.get(listUrl + listParam + pageParam).success(function (data) {
            $scope.loading = false;
            $scope.totalItems = data.totalCount;
            $scope.roles = data.resultList;
        }).error(function (data, status, headers, config) {
            toaster.pop('error', null, data.message);
        });
    };
...
App.controller('PrintShopController', ['$scope', '$http', 'toaster', 'ngDialog', '$filter', '$stateParams', '$q', '$timeout', function ($scope, $http, toaster, ngDialog, $filter, $stateParams, $q, $timeout) {
    $scope.pageSize = 20;//列表分页每页数
    $scope.currentPage = 1;
    $scope.printShop = {};
    $scope.provinces = [];
    ...
    $scope.listPrintShops = function () {
        ...
        var listParam = '?time=' + (new Date().getTime())
            ...
            + "&printerName=" + $scope.handleParam($scope.searchUserName)
            + "&printShopName=" + $scope.handleParam($scope.searchPrintShopName)
            + "&status=" + $scope.handleParam($scope.searchStatus)
            + "&provinceId=" + ($scope.province == undefined ? "" : $scope.province.id)
            + "&cityId=" + $scope.handleParam($scope.searchCity.id)
            + "&addressType=" + $scope.handleParam($scope.searchType)
            + "&createTimeFrom=" + (!$scope.createTimeFrom ? "" : $filter('date')($scope.createTimeFrom, 'yyyy-MM-dd'))
            + "&createTimeTo=" + (!$scope.createTimeTo ? "" : $filter('date')($scope.createTimeTo, 'yyyy-MM-dd'));
        $http.get(listUrl + listParam + pageParam).success(function (data) {
            $scope.loading = false;
     ...
     * 启用或禁用打印店
     */
    $scope.enOrDisablePrintShop = function (printShop) {
        if (printShop.status == 0) {
            $scope.deleteContent = "确认禁用该打印店吗？";
        } else {
            $scope.deleteContent = "确认启用该打印店吗？";
        }
        ...
        ngDialog.openConfirm({
            ...
            var data = {
                id: printShop.id,
                status: printShop.status == 0 ? 1 : 0
            };

            var url = printShop.status == 0 ? '/admin/printShop/disable' : '/admin/printShop/enable';
            $http
                .post(url, data)
                .success(function (data, status, headers, config) {
                    toaster.pop('success', null, data.message);
     ...
     * 修改打印店
     */
    $scope.savePrintShop = function () {
        $scope.printShop.provinceId = $scope.province.id;
        $scope.printShop.cityId = $scope.city.id;
        $scope.printShop.cityAreaId = $scope.area.id;

        var data = $scope.printShop;
        var url = $scope.printShop.id == undefined ? '/admin/printShop/save' : '/admin/printShop/update';
        $http
            .post(url, data)
            .success(function (data, status, headers, config) {
                toaster.pop('success', null, data.message);
     ...
     * @param printShop
     */
    $scope.initCitys = function (printShop) {

        $http.get('/common/city/listProvinceCityAreas?areaId=' + printShop.cityAreaId).success(function (data) {
            $scope.provinces = data.provinces;
            $scope.citys = data.citys;
            $scope.areas = data.areas;
            for (var i = 0; i < $scope.provinces.length; i++) {
                if (printShop.provinceId == $scope.provinces[i].id) {
            ...
            for (var i = 0; i < $scope.citys.length; i++) {
                if (printShop.cityId == $scope.citys[i].id) {
            ...
            for (var i = 0; i < $scope.areas.length; i++) {
                if (printShop.cityAreaId == $scope.areas[i].id) {
    ...
    $scope.upload = function () {
        var file = document.getElementById("shopImageFile");
    ...
    var fileSelect = function (evt) {
        ...
        var fileTyes = ["jpg", "png", "jpeg"];
        var fileType = fileTyes.includes(file.name.substr(file.name.lastIndexOf(".") + 1, file.name.length));
        if (!fileType || file.size > 2 * 1024 * 1024) {
            toaster.pop('error', null, '上传支持jpg, png, peg格式图片，不大于2M');
        }
        var reader = new FileReader();
        reader.onload = function (evt) {
            $scope.$apply(function ($scope) {
                $scope.printShop.printShopImage = evt.target.result;
            });
        };
        if (file)
            reader.readAsDataURL(file);
    };
    angular.element(document.querySelector('#shopImageFile')).on('change', fileSelect);
...
App.controller('ToasterDemoCtrl', ['$rootScope', '$scope', 'toaster', function ($rootScope, $scope, toaster) {

    $scope.toaster = {//error fail
        type: 'success',
        title: null,
        text: 'Message'
    };
...
App.controller('SidebarController', ['$rootScope', '$scope', '$state', '$http', '$timeout', 'Utils',
    function ($rootScope, $scope, $state, $http, $timeout, Utils) {
        ...
        $scope.addStaticMenu = function (menuItems, staticMenu) {
            var len = staticMenu.length;
            for (var i = 0; i < len; i++) {
                menuItems.push(staticMenu[i]);
            }
            ;
        }

        $scope.loadSidebarMenu = function () {
            //$rootScope.app.hostUrl + '/menu/getRoleMenu'
            //'server/sidebar-menu.json'
            $http.get('server/sidebar-menu.json').success(function (data) {
                $scope.staticMenu = data;
            });

            var menuJson = $rootScope.app.hostUrl + '/menu/getRoleMenu',
                menuURL = menuJson + '?v=' + (new Date().getTime()); // jumps cache
            $http.get(menuURL)
                .success(function (items) {
                    $scope.menuItems = items;
                    //添加静态菜单
                    $scope.addStaticMenu($scope.menuItems, $scope.staticMenu);
                })
                .error(function (data, status, headers, config) {
                    alert('Failure loading menu');
                });
